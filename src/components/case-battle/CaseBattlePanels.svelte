...
{#if countdownContent > 0 && countdownContent <= 3}
    <div class="absolute inset-0 z-50 flex items-center justify-center pointer-events-none">
      <div 
        class="relative flex h-32 w-32 items-center justify-center rounded-full border-2 border-gold shadow-[0_0_70px_rgba(255,215,0,0.5)] bg-[rgba(19,19,22,0.9)]"
      >
        {#key countdownContent}
          <div 
            class="absolute text-5xl font-black text-gold"
            ...
            {#if $page.data.user?.id === battleOwner}
              <div class="relative">
                <button
                  ...
                  on:click="{() => addBot(i)}"
                >
                  <svg class="icon -mt-0.5 mr-2 h-4 w-4 flex-shrink-0 text-white" viewBox="0 0 19 21" fill="none">
                    <path d="M10.5 3.055C15 3.552 18.5 7.367 18.5 12V21H0.5V12C0.5 7.367 4 3.552 8.5 3.055V0H10.5V3.055ZM9.5 17C10.8261 17 12.0979 16.4732 13.0355 15.5355C13.9732 14.5979 14.5 13.3261 14.5 12C14.5 10.6739 13.9732 9.40215 13.0355 8.46447C12.0979 7.52678 10.8261 7 9.5 7C8.17392 7 6.90215 7.52678 5.96447 8.46447C5.02678 9.40215 4.5 10.6739 4.5 12C4.5 13.3261 5.02678 14.5979 5.96447 15.5355C6.90215 16.4732 8.17392 17 9.5 17ZM9.5 15C8.70435 15 7.94129 14.6839 7.37868 14.1213C6.81607 13.5587 6.5 12.7956 6.5 12C6.5 11.2044 6.81607 10.4413 7.37868 9.87868C7.94129 9.31607 8.70435 9 9.5 9C10.2956 9 11.0587 9.31607 11.6213 9.87868C12.1839 10.4413 12.5 11.2044 12.5 12C12.5 12.7956 12.1839 13.5587 11.6213 14.1213C11.0587 14.6839 10.2956 15 9.5 15ZM9.5 13C9.76522 13 10.0196 12.8946 10.2071 12.7071C10.3946 12.5196 10.5 12.2652 10.5 12C10.5 11.7348 10.3946 11.4804 10.2071 11.2929C10.0196 11.1054 9.76522 11 9.5 11C9.23478 11 8.98043 11.1054 8.79289 11.2929C8.60536 11.4804 8.5 11.7348 8.5 12C8.5 12.2652 8.60536 12.5196 8.79289 12.7071C8.98043 12.8946 9.23478 13 9.5 13Z" fill="currentColor"></path>
                  </svg>
                  <span>{$_('battles.battlePage.playWithBot')}</span>
                </button>
                <button type="button" class="mx-auto mt-5 block text-sm uppercase {battleMode === 'underdog' ? 'text-purple-500' : 'text-neonGreen'}" on:click="{() => addAllBots()}">
            ...
            >
              {#each caseBattleData.drops[i][currentRound] as caseItem, j}
                <div
                  ...
                  style="backface-visibility: hidden; transform: rotateX({j *
                ...
                >
                  ...
                  {#if rollAnimationOver}
                    <div class="absolute bottom-2 left-1 right-1 bg-navy-900/90 rounded border border-navy-700 p-1 text-center pointer-events-none">
                      <div class="text-[8px] font-bold text-white truncate">{caseItem.weaponName}</div>
                      <div class="text-[7px] text-navy-300 truncate">{caseItem.skinName}</div>
                      <div class="text-[8px] text-green font-bold">{caseItem.skinQuality} • {convertPrice($page.data.currency, caseItem.skinPrice ?? 0)}</div>
                    </div>
                  {/if}
                </div>
              {/each}
