...
await playRollAnimation();
    winElmArr.forEach((elm, i) => {
      ...
      if (elm.querySelector('.award-img')) {
        // --- POPRAWIONA LOGIKA ---
        const drop = winningCaseDrops[i];
        const weapon = drop.weaponName.toLowerCase().replace(/\s+/g, '_');
        const skin = drop.skinName.toLowerCase().replace(/\s+/g, '_');
        
        // Ustawienie lokalnej ścieżki zamiast zewnętrznego URL
        (elm.querySelector('.award-img') as HTMLImageElement).src = `/skins/${weapon}_${skin}.png`;
...
<div
  ...
  style="height: 300px; box-shadow: rgba(66, 66, 84, 0.2) 0px 0px 0px 5px;"
>
  <div class="absolute left-1/2 top-1/2 z-10 flex h-5/6 -translate-x-1/2 -translate-y-1/2 scale-100 transform self-center justify-self-center transition-all duration-700">
    <div class="glow-gold ratio css-dap7rg group z-0 grid grid-cols-1 grid-rows-1 items-center justify-center justify-items-center rounded border border-solid border-gold bg-navy-700 bg-cover bg-center sm:rounded-lg" style="aspect-ratio: 5/3;">
      
      <div class="z-10 col-start-1 row-start-1 mb-auto flex w-full items-center justify-end px-11 py-2 text-right text-3xs font-semibold uppercase leading-none text-navy-200 sm:px-2 md:p-5">
        <div class="ml-2">Chance<br /><span class="award-chance"></span></div>
      </div>

      <!-- Bezpieczne ładowanie obrazka -->
      <img
        src={winningItem?.weaponName && winningItem?.skinName ? `/skins/${winningItem.weaponName.toLowerCase().replace(/\s+/g, '_')}_${winningItem.skinName.toLowerCase().replace(/\s+/g, '_')}.png` : ''}
        alt="{winningItem?.skinName || ''}"
      ...
      <div class="z-10 col-start-1 row-start-2 w-full self-end justify-self-start px-11 py-3 font-semibold uppercase leading-tight sm:px-3 md:row-start-1 md:p-5">
...
<ul class="CaseRolls-row flex h-full w-full">
          {#each rouletteItems as item}
            ...
            >
              <img src="/skins/{item.weaponName.toLowerCase().replace(/\s+/g, '_')}_{item.skinName.toLowerCase().replace(/\s+/g, '_')}.png" alt="" class="mx-auto h-full w-4/5 object-contain" />
              <div
...
<div class="CaseRolls-roll h-full w-full flex-shrink-0">
  {#each rouletteItemsArray as caseItem}
    ...
    >
      ...
      />
      <!-- Obrazek skina wyśrodkowany -->
      <img
        src="/skins/{caseItem.weaponName.toLowerCase().replace(/\s+/g, '_')}_{caseItem.skinName.toLowerCase().replace(/\s+/g, '_')}.png"
        alt="{caseItem.skinName}"
        style="width: 65%; height: 50%; object-fit: contain; z-index: 10;"
        class="block"
        loading="eager"
      />
      
      <!-- BLOK TEKSTOWY W LEWYM DOLNYM ROGU -->
      <div class="absolute left-2 bottom-2 z-20 flex flex-col items-start text-left leading-tight" style="max-width: 80%;">
        <!-- Nazwa broni -->
        <div class="truncate text-[12px] text-navy-200 uppercase font-sans">{caseItem.weaponName}</div>
        
        <!-- Nazwa skina -->
        <div class="truncate text-[14px] font-bold text-white uppercase font-sans mt-0.5">{caseItem.skinName}</div>
        
        <!-- Stan skina -->
        <div class="mt-1 rounded-[5px] bg-black/30 px-1.5 py-[1px] text-[7px] font-bold text-gold font-sans whitespace-nowrap">
          {caseItem.skinQuality || caseItem.wear || 'FACTORY NEW'}
        </div>
      </div>
