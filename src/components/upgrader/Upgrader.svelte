...
function getSkinPath(weapon: string, skin: string) {
    ...
    const w = weapon.toLowerCase().replace(/[^a-z0-9]/g, '_');
    const s = skin.toLowerCase().replace(/[^a-z0-9]/g, '_');
...
<div class="flex flex-wrap items-center justify-center gap-2">
              {#each selectedUpgradeItems as item}
                <img
                  ...
                  on:keydown="{null}"
                  class="max-w-[18%] flex-shrink-0 cursor-pointer object-contain transition-all hover:scale-105"
                  src="{getSkinPath(item.globalInvItem.weaponName, item.globalInvItem.skinName)}"
                  alt="{item.globalInvItem.skinName}"
          ...
          >
            {#each selectedGoalItems as item}
              <img
                ...
                src="{getSkinPath(item.weaponName, item.skinName)}"
                alt="{item.skinName}"
                on:error={(e) => (e.currentTarget.src = '/images/placeholder.png')}
              />
            {/each}
